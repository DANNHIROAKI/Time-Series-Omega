...
class SFFOmega(nn.Module):
    ...
    def canonicalise(
        ...
        warped_series = self.time_warp.apply(series, mask=mask)
        ...
        _, diagnostics = self.time_warp.forward(return_diagnostics=True)  # type: ignore[misc]
        return GaugeOutputs(
    ...
    def regularisation(
        ...
        reg["h_disc"] = torch.mean(
            (gauge.warped - low_pass_filter(gauge.warped, cutoff=0.45)) ** 2
        )
        return reg
